...
//
// ESM module variant for AUTO-MOUNTING in every session via an agent preset:
//   - copy a shipped preset into the user root (`~/.dsh/.agent-presets/<id>/`)
//   - place this file at `<preset>/plugin/memory-system-hooks.mjs`
//   - add a row to the copy's `agent.cordis.yml`:
//       - id: memory-system-hooks
//         name: './plugin/memory-system-hooks.mjs'
//   - start sessions on that preset; the plugin mounts automatically.
//
// (The sibling `memory-system-hooks.js` holds the dynamic cordis_define body,
//  `return { … }`, for one-off/session-scoped mounting.)
//
// The plugin enforces the skill's mandatory automatic executions:
...
//
// Directory spec (skill): `<repo-root>/.memory/{version}/{date}/{sessionId}`
// where `sessionId` is AGENT-SPECIFIED. Resolution order: the id the agent
// gave `memory_sync_now(sessionId)` (remembered and reused by the hooks) →
// a VALID slug from the session title → the last sessionId persisted for this
// repo (continuity across compaction, where the dsh session id and title both
// change) → a deterministic valid hash fallback. Every fallback is validated
// against the skill's naming rule, so the hooks never write an id that the
// `memory_sync_now` tool would reject (bare number / `session-<hex>` hash).
...
//
// Writes carry the session's resolved sandbox policy, so under the default
// workspace-write mode the plugin persists `<session-cwd>/.memory/…` within
// the same confinement as the agent's own tools.
  ...
  apply(ctx) {
    ...
    tools?.register({
      ...
      async execute(args, exec) {
        ...
        await syncSession(exec.agent, 'tool:memory-sync-now');
    ...
    tools?.register({
      ...
      parameters: {
        type: 'object',
        properties: {
          version: { type: 'string', description: 'Memory format version to write under, in v<major>.<minor>.<patch> form (e.g. \'v0.0.2\'). Empty string clears the pin (back to latest).' },
        },
        required: ['version'],
      },
      output: { schema: { type: 'string' }, render(_a, v) { return [{ type: 'text', text: v }] } },
      async execute(args, exec) {
        const s = sessionOf(exec?.agent);
        if (!s || !s.cwd) return 'no live agent session';
        ...
        const repoRoot = await findRepoRoot(s.cwd);
        const sessionId = await resolveSessionId(exec.agent, s.dshId, repoRoot);
